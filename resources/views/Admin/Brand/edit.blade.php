...
@section('content')
    <div class="content-wrapper">
        ...
        <section class="content">
            <!-- Default box -->
            <form action="{{ route('admin.brand.update', $brands->id) }}" method="post" id="BrandForm">
                @csrf
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select name="status" id="status" class="form-control">
                                            <option {{ $brands->status == 1 ? 'selected' : '' }} value="1">Active
                                            </option>
                                            <option {{ $brands->status == 0 ? 'selected' : '' }} value="0">Block
                                            </option>
                                        </select>
                                    </div>
                                </div>
...
@section('jsPage')
    <script>
        $('#BrandForm').submit(function(event) {
            ...
            $.ajax({
                url: '{{ route('admin.brand.update', $brands->id) }}',
                type: 'put',
